...
public class homeActivity extends AppCompatActivity implements home.OnProductClickListener, Milktea.OnProductClickListener, fruit_tea.OnProductClickListener, frappe.OnProductClickListener, milkshake.OnProductClickListener, coolers.OnProductClickListener, sandwich.OnProductClickListener, pasta.OnProductClickListener, pizza.OnProductClickListener, mozzarella.OnProductClickListener, fries.OnProductClickListener, cheese_sticks.OnProductClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout FeedsubMenu = findViewById(R.id.FeedsubMenu);


        faq = findViewById(R.id.faq);

        for (int i = 0; i < subMenu.getChildCount(); i++) {
            subMenu.getChildAt(i).setVisibility(View.GONE);
        }
        for (int i = 0; i < FeedsubMenu.getChildCount(); i++) {
            FeedsubMenu.getChildAt(i).setVisibility(View.GONE);
        }

        aboutClick.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                boolean isVisible = subMenu.getChildAt(0).getVisibility() == View.VISIBLE;

                // Close FeedsubMenu if open
                if (FeedsubMenu.getChildAt(0).getVisibility() == View.VISIBLE) {
                    for (int i = 0; i < FeedsubMenu.getChildCount(); i++) {
                        final View child = FeedsubMenu.getChildAt(i);
                        child.animate()
                                .alpha(0f)
                                .setDuration(500)
        ...
        feedback.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                boolean isVisible = FeedsubMenu.getChildAt(0).getVisibility() == View.VISIBLE;
